...
#include "knnquery.h"
#include "method/hcnsw.h"
#include "method/hcnsw_distfunc_opt_impl_inline.h"
...
namespace similarity {
    ...
    float
    NegativeDotProduct2(const float *pVect1, const float *pVect2, size_t &qty, float * __restrict TmpRes) {
     ...
     */
    float
    NormCosine2(const float *pVect1, const float *pVect2, size_t &qty, float *__restrict TmpRes) {
        return std::max(0.0f, 1 - std::max(float(-1), std::min(float(1), ScalarProduct(pVect1, pVect2, qty, TmpRes))));
    }

    float L1NormWrapper2(const float *pVect1, const float *pVect2, size_t &qty, float *) {
        return L1NormSIMD(pVect1, pVect2, qty);
    }

    float LInfNormWrapper2(const float *pVect1, const float *pVect2, size_t &qty, float *) {
        return LInfNormSIMD(pVect1, pVect2, qty);
    }

    EfficientDistFunc getDistFunc(DistFuncType2 funcType) {
        switch (funcType) {
            case kL2Sqr16Ext2 : return L2Sqr16Ext;
            case kL2SqrExt2   : return L2SqrExt;
            case kNormCosine2 : return NormCosine2;
            case kNegativeDotProduct2 : return NegativeDotProduct2;
            case kL1Norm2 : return L1NormWrapper2;
            case kLInfNorm2 : return LInfNormWrapper2;
        }
...
// This is the counter to keep the size of neighborhood information (for one node)
    ...
    Hcnsw<dist_t>::Hcnsw(bool PrintProgress, const Space<dist_t> &space, const ObjectVector &data)
        : Index<dist_t>(data)
        , space_(space)
    ...
    void
    checkList1(vector<HcnswNode *> list)
    {
        int ok = 1;
    ...
    void
    getDegreeDistr(string filename, vector<HcnswNode *> list)
    {
        ofstream out(filename);
        size_t maxdegree = 0;
        for (HcnswNode *node : list) {
        ...
        for (size_t i = 0; i < list.size(); i++) {
            for (HcnswNode *node : list[i]->allFriends_[0]) {
    ...
    template <typename dist_t>
    void
    Hcnsw<dist_t>::CreateIndex(const AnyParams &IndexParams)
    {
        ...
        visitedlistpool = new VisitedListPool2(indexThreadQty_, this->data_.size());
...
// int degrees[100] = {0};
            ParallelFor(1, this->data_.size(), indexThreadQty_, [&](int id, int threadId) {
                HcnswNode *node1 = ElList_[id];
                HcnswNode *node2 = temp[id];
                vector<HcnswNode *> f1 = node1->getAllFriends(0);
                vector<HcnswNode *> f2 = node2->getAllFriends(0);
                unordered_set<size_t> intersect = unordered_set<size_t>();
                for (HcnswNode *cur : f1) {
                    intersect.insert(cur->getId());
                }
                for (HcnswNode *cur : f2) {
                    intersect.insert(cur->getId());
                }
                if (intersect.size() > maxF)
                    maxF = intersect.size();
                vector<HcnswNode *> rez = vector<HcnswNode *>();
        ...
        dist_func_type_ = kDistTypeUnknown2;
        ...
        CHECK(dist_func_type_ != kDistTypeUnknown2);
    ...
    template <typename dist_t>
    void
    Hcnsw<dist_t>::SetQueryTimeParams(const AnyParams &QueryTimeParams)
    {
        ...
        pmgr.CheckUnused();
        LOG(LIB_INFO) << "Set HCNSW query-time parameters:";
    ...
    template <typename dist_t>
    const std::string
    Hcnsw<dist_t>::StrDesc() const
    {
        return METH_HCNSW;
    }

    template <typename dist_t> Hcnsw<dist_t>::~Hcnsw()
    {
    ...
    template <typename dist_t>
    void
    Hcnsw<dist_t>::kSearchElementsWithAttemptsLevel(const Space<dist_t> *space, const Object *queryObj, size_t efConstruction,
                                                   priority_queue<HcnswNodeDistCloser<dist_t>> &resultSet, HcnswNode *ep,
                                                   int level) const
    {
#if EXTEND_USE_EXTENDED_NEIGHB_AT_CONSTR != 0
        priority_queue<HcnswNodeDistCloser<dist_t>> fullResultSet;
#endif

#if USE_BITSET_FOR_INDEXING
        VisitedList2 *vl = visitedlistpool->getFreeVisitedList();
        vl_type *mass = vl->mass;
        vl_type curV = vl->curV;
#else
        unordered_set<HcnswNode *> visited;
#endif
        HcnswNode *provider = ep;
        priority_queue<HcnswNodeDistFarther<dist_t>> candidateSet;
        dist_t d = space->IndexTimeDistance(queryObj, provider->getData());
        HcnswNodeDistFarther<dist_t> ev(d, provider);
        ...
        while (!candidateSet.empty()) {
            const HcnswNodeDistFarther<dist_t> &currEv = candidateSet.top();
            ...
            */
            ...
            HcnswNode *currNode = currEv.getMSWNodeHier();
            ...
            */
            ...
            const vector<HcnswNode *> &neighbor = currNode->getAllFriends(level);
            ...
            for (auto iter = neighbor.begin(); iter != neighbor.end(); ++iter) {
                ...
                if (visited.find((*iter)) == visited.end()) {
                    ...
                    HcnswNodeDistFarther<dist_t> evE1(d, *iter);
    ...
    template <typename dist_t>
    void
    Hcnsw<dist_t>::Search(RangeQuery<dist_t> *query, IdType) const
    {
    ...
    template <typename dist_t>
    void
    Hcnsw<dist_t>::Search(KNNQuery<dist_t> *query, IdType) const
    {
        ...
        case 0:
            ...
            if (useOld)
                const_cast<Hcnsw *>(this)->baseSearchAlgorithmOld(query);
            else
                const_cast<Hcnsw *>(this)->baseSearchAlgorithmV1Merge(query);
            break;
        case 3:
        case 4:
            /// Basic search using optimized index for l2, cosine, negative dot product
            if (useOld)
                const_cast<Hcnsw *>(this)->SearchOld(query, iscosine_);
            else
                const_cast<Hcnsw *>(this)->SearchV1Merge(query, iscosine_);
            break;
        default:
    ...
    template <typename dist_t>
    void
    Hcnsw<dist_t>::SaveIndex(const string &location) {
    ...
    template <typename dist_t>
    void
    Hcnsw<dist_t>::SaveOptimizedIndex(std::ostream& output) {
    ...
    template <typename dist_t>
    void
    Hcnsw<dist_t>::SaveRegularIndexBin(std::ostream& output) {
        totalElementsStored_ = ElList_.size();
        ...
        for (unsigned i = 0; i < totalElementsStored_; ++i) {
            const HcnswNode& node = *ElList_[i];
    ...
    template <typename dist_t>
    void
    Hcnsw<dist_t>::SaveRegularIndexText(std::ostream& output) {
    ...
    template <typename dist_t>
    void
    Hcnsw<dist_t>::LoadRegularIndexText(std::istream& input) {
        ...
        for (unsigned id = 0; id < totalElementsStored_; ++id) {
            ElList_[id] = new HcnswNode(this->data_[id], id);
        }
    ...
    template <typename dist_t>
    void
    Hcnsw<dist_t>::LoadRegularIndexBin(std::istream& input) {
        LOG(LIB_INFO) << "Loading regular index.";
    ...
    template <typename dist_t>
    void
    Hcnsw<dist_t>::LoadIndex(const string &location) {
        ...
        visitedlistpool = new VisitedListPool2(1, totalElementsStored_);
    ...
    template <typename dist_t>
    void
    Hcnsw<dist_t>::LoadOptimizedIndex(std::istream& input) {
    ...
    template <typename dist_t>
    void
    Hcnsw<dist_t>::baseSearchAlgorithmOld(KNNQuery<dist_t> *query)
    {
        VisitedList2 *vl = visitedlistpool->getFreeVisitedList();
        ...
        dist_t curdist = d;
        HcnswNode *curNode = provider;
        for (int i = maxlevel1; i > 0; i--) {
            bool changed = true;
            while (changed) {
                changed = false;

                const vector<HcnswNode *> &neighbor = curNode->getAllFriends(i);
                for (auto iter = neighbor.begin(); iter != neighbor.end(); ++iter) {
                    PREFETCH((char *)(*iter)->getData(), _MM_HINT_T0);
                }
                for (auto iter = neighbor.begin(); iter != neighbor.end(); ++iter) {
        ...
        priority_queue<HcnswNodeDistFarther<dist_t>> candidateQueue;   // the set of elements which we can use to evaluate
        priority_queue<HcnswNodeDistCloser<dist_t>> closestDistQueue1; // The set of closest found elements

        HcnswNodeDistFarther<dist_t> ev(curdist, curNode);
        ...
        while (!candidateQueue.empty()) {
            ...
            const HcnswNodeDistFarther<dist_t> &currEv = iter;
            ...
            HcnswNode *initNode = currEv.getMSWNodeHier();
            candidateQueue.pop();

            const vector<HcnswNode *> &neighbor = (initNode)->getAllFriends(0);
    ...
    template <typename dist_t>
    void
    Hcnsw<dist_t>::baseSearchAlgorithmV1Merge(KNNQuery<dist_t> *query)
    {
        VisitedList2 *vl = visitedlistpool->getFreeVisitedList();
        vl_type *massVisited = vl->mass;
        vl_type currentV = vl->curV;
        ...
        SortArrBI<dist_t, HcnswNode *> sortedArr(max<size_t>(ef_, query->GetK()));
        sortedArr.push_unsorted_grow(curdist, curNode);

        int_fast32_t currElem = 0;

        typedef typename SortArrBI<dist_t, HcnswNode *>::Item QueueItem;
    ...
    template class Hcnsw<float>;
    template class Hcnsw<int>;
}
