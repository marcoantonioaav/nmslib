...
namespace similarity {
    ...
    void
    Hcnsw<dist_t>::SearchOld(KNNQuery<dist_t> *query, bool normalize)
    {
        ...
        priority_queue<EvaluatedMSWNodeInt2<dist_t>> candidateQueuei; // the set of elements which we can use to evaluate

        priority_queue<EvaluatedMSWNodeInt2<dist_t>> closestDistQueuei; // The set of closest found elements
        // EvaluatedMSWNodeInt2<dist_t> evi(curdist, curNodeNum);
        ...
        while (!candidateQueuei.empty()) {
            EvaluatedMSWNodeInt2<dist_t> currEv = candidateQueuei.top(); // This one was already compared to the query
    ...
    template <typename dist_t>
    void
    Hcnsw<dist_t>::SearchV1Merge(KNNQuery<dist_t> *query, bool normalize)
    {
        float *pVectq = (float *)((char *)query->QueryObject()->data());
        TMP_RES_ARRAY(TmpRes);
        size_t qty = query->QueryObject()->datalength() >> 2;
